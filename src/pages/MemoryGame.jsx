...
export default function MemoryGame() {
  ...
  // Local state to control win message visibility
  ...
  // Show win message when game won flag changes
  useEffect(() => {
    if (isGameWon) {
  ...
  // Handle Next Level click with fade out of win message
  function handleNextLevel() {
    // Fade out message first
    setShowWinMessage(false);

    // After delay, dispatch nextLevel
    setTimeout(() => {
      dispatch(nextLevel());
    }, 600); // same as animation duration for smooth transition
  }

  // Confetti animation variants (same as before)...
  ...
  return (
    <motion.div
      className="flex flex-col items-center p-6 min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    >
            ...
            >
              <motion.div
                ...
                animate={{
                  ...
                  scale: flipped.length === 2 && !card.matched ? [1, 1.05, 1] : 1,
                }}
